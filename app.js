...
class AElfEgg {
  ...
  initInstance(url) {
    ...
    httpProvider[0] = httpProvider[0] || url;
  ...
  initContractInstance(apiInfo) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let httpProvider = Array.from(this.httpProvider);
      httpProvider[0] = httpProvider[0] || urlTemp;

      const aelf = new AElf(new AElf.providers.HttpProvider(...httpProvider));
      aelfInstances[httpProvider[0]] = aelf;

      aelf.chain.contractAt(contract_address, wallet).then(contract => {
        aelfContractInstances[contract_address + chain_id] = contract;
        resolve(contract);
      }).catch(error => {
        reject(error);
      });
      // aelf.chain.contractAt(contract_address, wallet, (err, contract) => {
  ...
  createAelf(rpcApiList) {
    ...
    const initTokenContract = inputOptions => {
      ...
      this.initContractInstance(apiInfo).then(() => {
        requestCount--;
